...
case class TrialOC(val id: Int = Int.MinValue, val version: Int = 0, val identifier: String, val oid: String, val name: String, val description: String, val metaDataVersionOID: String = "", val events: List[EventOC] = List(), val trial: Option[Trial], connection: ConnectionOC, val treatmentItem: Option[(EventOC, FormOC, ItemGroupOC, ItemOC)], val sites: Map[String, String] =  Map()) extends Entity {
